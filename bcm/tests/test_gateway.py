...
def run_test():
    ...
    bytes_out, _ = gw.process_and_send(null_payload, wbp_payload, flash_state=False)
    ...
    bytes_out_tick, _ = gw.process_and_send(left_turn_payload, wbp_payload, flash_state=True)
    print(f"CAN Payload - Tick: {bytes_out_tick.hex()}")

    print("\n--- TEST 3: LEFT TURN ON (Tock) ---")
    # Flash OFF, but Left Button is still ON (being held down)
    bytes_out_tock, _ = gw.process_and_send(left_turn_payload, wbp_payload, flash_state=False)
    ...
    bytes_out_head, _ = gw.process_and_send(low_beam_left_turn_payload, wbp_payload, flash_state=True)
